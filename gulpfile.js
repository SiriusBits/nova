 ...
 */
gulp.task('clean:styles', () => del([`${outputPaths.css}/style.css`, `${outputPaths.css}/style.min.css`]));
...
gulp.task('clean:markup', () =>
  del([`${outputPaths.markup}/**/*`, `!./${outputPaths.markup}/assets`, `!./${outputPaths.markup}/assets/**/*`])
);
...
gulp.task('start-server', cb => {
  ...
  return nodemon({
    script: 'bin/www',
    ignore: ['gulpfile.js', 'webpack.config.js', 'src/', 'dist/', 'node_modules/'],
  })
    .on('start', () => {
      if (!called) {
        called = true;
...
gulp.task('publish-static', ['build-html', 'set-production', 'build:assets'], () =>
  gulp
...
gulp.task('clean:all', ['clean-fonts', 'clean-styles', 'clean-scripts', 'clean-images', 'clean-markup'], () => {});
